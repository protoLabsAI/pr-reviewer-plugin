...
def _unreviewed_prs(exhaustions: list[dict], reviewed: list[dict], limit: int = 20) -> dict:
    ...
    exhaustion already covered by a later verdict is not outstanding.

    NOTE this is stricter than "never reviewed at all": a verdict on an EARLIER head does
    not count as covering the head that failed. On the fleet reviewer that difference is
    33 vs 24 over five weeks, and the stricter reading is the one a gate wants.
    """
...
def build_report(events: list[dict]) -> dict:
    ...
    return {
        ...
        # The OUTCOME of those exhaustions (issue #54). #61 made an exhausted panel
        # visible ON the PR; this makes the backlog visible to a CRON. `exhaustions`
        # alone cannot distinguish a panel that failed and was recovered by the sweep's
        # backfill — noise — from one whose PR merged past the gate with no verdict ever,
        # which is the gate silently not existing. Over the fleet reviewer's first five
        # weeks the split was 44 exhausted PRs / 24 never reviewed, and nothing surfaced
        # that second number: finding it took reading the operator inbox by hand, five
        # weeks late. This is the field to alert on.
...
def render_report_markdown(summary: dict, rows: list[dict] | None = None) -> str:
    ...
    lines = [
        ...
        # Units are named explicitly: `exhaustions` counts EVENTS (a PR can exhaust more
        # than once), while the two figures beside it count distinct PRs keyed on their
        # last exhaustion. Reading "61 exhaustions, 33 still unreviewed" as a subset of
        # one another is wrong, and the earlier phrasing invited exactly that.
        f"- **Exhaustions (fail-closed, no verdict):** {summary.get('exhaustions', 0)} event(s) — "
        f"**{(summary.get('unreviewed_prs') or {}).get('count', 0)} PR(s) still have no verdict**, "
        f"{(summary.get('unreviewed_prs') or {}).get('recovered', 0)} PR(s) recovered by a later pass",
